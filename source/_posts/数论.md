...
贝祖定理是有关于最大公约数的定理，具体公式：
$$
ax+by = gcd(a,b)
$$
这个定理表明，对于任何两个整数a和b，都可以找到一对整数x和y，使得它们的线性组合等于它们的最大公因数。
...
### [线性同余方程](https://zhuanlan.zhihu.com/p/451193655)

形如
$$
ax≡b (mod n)
$$
其中`a,b,n`都为常量，x是未知数，这个方程可以化为(取模n - 变为方程)
$$
ax = kn+b
$$
这里的k为任意常数，于是一般式为：
...
线下同余方程可以化为一般式：`ax+by+c = 0`

首先用`gcd()`求出`a,b`的最大公约数`d`,那么方程可以化为`d(ax/d+by/d) = c`,容易得到`(ax/d+by/d)`为整数，如果`d不能整除c`，则方程必然无解,算法结束，否则进入拓展欧几里得算法。

#### [扩展欧几里得算法](https://zhuanlan.zhihu.com/p/100567253)*

在前面补充了，线性同余方程可以化为一般式：`ax+by+c = 0`

那么，什么是**拓展欧几里得**呢？这是一种算法，它可以在辗转相除途中求出不定方程`ax+by = c`的一组解。

![img](v2-9c9e1575adfbd3a78a25e3788f73b6b7_720w.png)

这张图是欧几里得算法的流程，对这个流程进行分析。

举个例子：上图倒数第二行 `3+6*3 = 21`,可以改写为`3 = 6*(-3)+21`. 也就是说3可以被表示为`6和21`的线性组合(就是这三个数可以凑成等式嘛)

然后倒数第三行，`6+21*1 = 27`,说明6可以表示为21和27的线性组合，那么3也可以表示为21和27的线性组合。有：
$$
3 = 6*(-3)+21 = (27-21)*(-3)+21 = 27*(-3)+21*4
$$
这样一路推下去，即知 3 可以表示为 75 和 48 的线性组合。那么 `75x+48y=3 `就能找到解了。

![image-20230906224409332](image-20230906224409332.png)
