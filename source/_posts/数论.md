...
>  特别的如果`gcd(a,b) = 1`说明我们的`a,b`只存在一个公因数`1`，表明`a,b`互质
...
>
> 为什么第一个参数要大于第二个参数呢，因为最后出现的0不能作为除数。如果还是不能理解，就记住公式就行吧。

### [扩展欧几里得](https://zhuanlan.zhihu.com/p/100567253)*(不会能用就行)
...
拓展欧几里得可以用于求解线性同余方程`ax≡b (mod n)`,实际上拓展欧几里得是在求解最大公约数的时候对其加上一些限制，达到解决问题的目的。下面是求解方程`ax≡b (mod n)`的具体步骤：
...
`1010`的最后一位是`0`，所以`a^1`这一位不要。然后`1010`变为`101`，`a`进位变为`a^2`。

`101`的最后一位是`1`，所以`a^2`这一位是需要乘的，乘入`ans`。`101`变为`10`，`a`再自乘进位`a^3`。

`10`的最后一位是`0`，跳过，右移，自乘进位。

`1`的最后一位是`1`，`ans`再乘上`a^8`。循环结束，返回结果。
...
数学上的乘法逆元就是一个数的倒数，既a的逆元是`1/a`,也就是和a相乘得到1的数据。`ax = 1`,则x是a的乘法逆元。

> 发明乘法逆元的目的是为了将除法变为乘法，因为计算除法很麻烦。

这里讨论有关于取模运算的乘法逆元，既对于整数a,与a互质的数b作为模数，当整数x满足`ax mod b ≡ 1`时，称`x`为`a`关于模`b`的逆元，也就是`a * x % b == 1`
...
- 逆元： a关于模b的逆元 整数d满足`ad mod b ≡ 1`
- 拓展欧几里得：求方程`ax+by = gcd(a,b)`的一组可行解

逆元的`ad mod b ≡ 1`, 等价于 `ad - kb = 1`,其中k为未知整数。
