...
> 用于遍历数据结构的指针，也需要**和对应数据结构的类型相同**。就像是量我们的铅笔，我们需要用厘米尺而不是分米尺。
...
1. 数组：按一定格式排列起来的，具有**相同类型**的数据元素集合。
2. 一般都是采用顺序存储结构来表示数组。
3. 数组可以是多维的，但存储数据元素的内存单元地址是一维的，因此，在存储数组结构之前，需要解决将多维关系映射到一维关系的问题。
4. 结论：**线性表结构是数组结构的一个特例，而数组结构又是线性表结构的扩展。**
5. 特点：结构固定——定义后，**维数和维界不再改变**。
6. **n维数组：若 n-1 维数组中的元素又是一个一维数组结构，则称为n维数组。**
...
2. 一维数组的逻辑结构：
   - 线性结构（定长的线性表）
3. 声明格式：数据类型 变量名称[长度]
4. 各数组元素大小相同，且在**物理**上连续存放。

![img](./3027548-20230805140439283-1972200470.png)

#### 二维数组

1. 二维数组：若**一维数组中的元素又是一个一维数组结构**，则称为二维数组。

2. 二维数组的逻辑结构：

   - 非线性结构：每一个**数据元素即在一个行表中，又在一个列表中**。

   - 线性结构（定长的线性表）：该线性表的每个数据元素也是一个定长的线性表。

3. 声明格式：数据类型变量名称[行数] [列数]

4. 其内存表示分为两种不同的存储策略，一种是**行优先存储**，一种是**列优先存储：**

   > **行优先存储，即一行一行的存：**

![img](./3027548-20230805140539848-1796666549.png)

![img](./3027548-20230805140559480-472182394.png)

> **列优先存储，即一列一列的存：**

![img](./3027548-20230805140610109-490304831.png)

![img](./3027548-20230805140622650-990246509.png)
...
映射：数组的下标0与1与矩阵`Aᵢ Aⱼ`的元素下标i和j的关系就是所谓的映射

- 矩阵的压缩存储：在一个矩阵中，若多个数据的值相同，为了节省内存空间，则只分配一个元素值的空间，且零元素不占存储空间。
- 公式：
  1. 矩阵非零元素A(i)(j)在一维数组B中求对应下标
     `Loc[i,j]=Loc[1,1]+(a(i)(j)前面的非零元素个数））` --> `Loc[1,1]`代表的是首地址
  2. 矩阵非零元素a(i)(j)在一维数组B中地址(size是单个元素的字节大小）
     `Loc[A(i)(j)]=Loc[A(1,1)]+(a(i)(j)前面的非零元素个数))∗size`

#### 对称矩阵

- 对称矩阵是指元素以主对角线为对称轴对应相等的n阶矩阵，即`aᵢⱼ=aⱼᵢ` （ **注意，n阶的意思是行数和列数相同。**）
- 只存储下（或者上）三角（包括主对角线）的数据元素。
- 共占用 `n(n+1) / 2 `个元素空间。
- **按行优先原则将各个元素存入一维数组中：**

![img](./3027548-20230805140703045-672749092.png)

- ***按列优先原则将各个元素存入一维数组中：**

![img](./3027548-20230805140756877-740544349.png)

- **注意点：**

![img](./3027548-20230805142246980-1251000188.png)

![img](./3027548-20230805142214478-1907580443.png)
...
![img](./3027548-20230805141126801-919542494.png)

![img](./3027548-20230805140916454-1137237661.png)

##### 上三角矩阵

![img](./3027548-20230805141140722-2034067682.png)

![img](./3027548-20230805140932057-23359145.png)
...
- 在 n 阶矩阵中，所有非零元素都**集中在以主对角线为中心的带状区域中**，区域外的值全为0。
- 常见的有：三对角矩阵、五对角矩阵、七对角矩阵。
- 非零`a(i)(j)`特点；
  1. i = 1(第一行，只有两个元素）：j = 1, 2
  2. 1 < i < n（中间行，每行有三个元素，j有三个取值）：j = `i - 1`, `i` , `i +1`
  3. i = n (最后一行，只有两个元素）： j = `n-1`, `n`
- 压缩方法：
  1. 确定存储该矩阵所需的一维存储空间（即一维数组B）的空间大小
     每一行假设都有3个元素，但是由于第一行和最后一行只有两个，所以一维数组的容量为：`3n - 2`
  2. 确定非零元素a(i)(j)在一维空间的下标（地址)
...
**由此得到：A(i)(j)在B数组中的下标为（其中`Loc(1)(1) = 0 Loc[i,j]=Loc[1,1]+(2(i−1)+j−1`）**
...
- 就是给每个非零元素在存储自身的值的基础上再加两个域。
  1. 行域：存储这个非零元素在原来的的矩阵中所占的**行号**。
  2. 列域：存储这个非零元素在原来的矩阵中所占的**列号**。
  3. 值域：存储原来的非零元素的值。
...
- 以**十字链表**法进行**链式存储**
- 每一个非零元素节点，不仅有这3个域：**`（row , col , value)`**.还增加了两个链域：
  1. **`right:`链接同一行中的下一个非零元素**
  2. **`down:`链接同一列中的下一个非零元素**
...
![img](./3027548-20230805141657272-2054492344.png)
...
![img](./3027548-20230805141226900-491201429.png)

![img](./3027548-20230805141241240-483025010.png)

#### 广义表的深度

广义表的深度，可以通过观察该表中所包含括号的层数间接得到，如下示例，该广义表的深度为2。注意点：递归广义表的**深度是无穷大！！！**

![img](./3027548-20230805141524851-1800779053.png)
...
- 计算元素个数时，广义表中存储的每个原子算作一个数据，同样每个子表也只算作是一个数据。
  1. LS = (`a1,a2,…,an`) 的长度为 n；
  2. 广义表 (a,(b,c,d)) 的长度为 2；
  3. 广义表 ((a,b,c)) 的长度为 1；
  4. 空表 () 的长度为 0。
  5. 表 (()) 的长度为1，表头与表尾均为 ()。
- 求广义表长度时，如下示意图所示：

![img](./3027548-20230805141501872-2039631144.png)
...
- 广义表可以为其它广义表共享！
- 广义表是**多层次**结构，**其里面的元素可以是单元素，也可以是子表，而子表的元素还可以是子表**。
